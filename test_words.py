...
def test_suffix():
    ...
    assert suffix("cat", "catalog") == "catalog"
    assert suffix("", "") == ""
    assert suffix("", "correct") == "correct"
    assert suffix("clear", "") == "clear"
    assert suffix("happy", "funny") == "funny"
    assert suffix("cat", "catalog") == "catalog"
    assert suffix("dogmatic", "dog") == ""
    assert suffix("jump", "joyous") == "ous"
    assert suffix("upbeat", "upgrade") == "grade"
    assert suffix("Disable", "dIstasteful") == "steful"



# Call the main function that is part of pytest so that the
# computer will execute the test functions in this file.
pytest.main(["-v", "--tb=line", "-rN", pytest])
